...
function cleanConsoleForUser(text) {
  ...
  const ok = lines.filter((l) => /\bOK\b|OK_/.test(l)).length;
...
function App() {
  ...
  const summary = useMemo(() => {
    ...
    const fail = (output.match(/\[FAIL\]|\bFAIL: [1-9]|CORTO_SOSPECHOSO/g) || []).length;
  ...
  async function analyzeBoard() {
    if (!file) { setError("Selecciona un BSDL primero."); return; }
        ...
        <div className="grid cleanGrid">
          <section className="card controls compactControls">
            ...
            {uploadOpen && <div className="uploadPanel">
              <div className="inlineFile"><label>BSDL</label><input type="file" accept=".bsdl,.bsd,.txt" onChange={(e) => setFile(e.target.files?.[0] || null)} disabled={running} /><span>{file ? file.name : "no seleccionado"}</span></div>
              <div className="inlineFile"><label>NET</label><input type="file" accept=".net,.cir,.csv,.txt,.xml" onChange={(e) => setNetlistFile(e.target.files?.[0] || null)} disabled={running} /><span>{netlistFile ? netlistFile.name : "opcional"}</span></div>
            ...
            <div className="buttonStack"><button className="secondary" onClick={analyzeBoard} disabled={running || !file}>Analizar / cargar pines</button><button className="primary" onClick={startTest} disabled={running || !file}>{running ? "Revisión en curso..." : "Revisión completa"}</button></div>
